...
-- ▼ 위에서 조회한 각각의 테이블들이 어떤 테이블스페이스에 저장되어 있는지 조회
...
-- ▼ TBL_EXAMPLE1과 TBL_EXAMPLE2 테이블이 각각 어떤 테이블스페이스에 저장됐는지 조회
SELECT TABLE_NAME, TABLESPACE_NAME
FROM USER_TABLES;
/*
DEPT	USERS
...
FROM TAB;
/*
BONUS	        TABLE	-- 보너스(BOUNUS) 데이터 테이블
...
2. 문자형 CHAR         → 고정형 크기 (무조건 지정된 크기 소모)
          Ex)
          CHAR(10)     ← '강의실'        ▶ 6Byte 짜리지만 10Byte를 소모
                       ← '실습조영관'    ▶ 10Byte
                       ← '영관이뭐하지'  ▶ 10Byte를 초과하여 입력 불가

          VARCHAR2     → 가변형 크기 (상황에 따라 크기 변경)
          Ex)
          VARCHAR2(10) ← '박원석'        ▶ 6Byte 소모
                       ← '실습조영관'    ▶ 10Byte
                       ← '영관이뭐하지'  ▶ 10Byte를 초과하여 입력 불가
                       
            ▶ VARCHAR2는 자료구조적으로 CHAR보다 비효율적!
               그래서 두 가지로 구분해서 사용한다.
...
-- ▼ EMP 테이블에서 사원번호, 사원명, 급여, 커미션 데이터만 조회
DESCRIBE EMP;
...
FROM EMP;
-- ▶ 테이블을 조회하는 과정에서 각 컬럼의 이름에는 별칭(ALIAS) 부여 가능
--    기본 구분 형식은 『컬럼명 AS "별칭이름"』의 형태로 작성되며,
--    이때 『AS』는 생략이 가능하다.
--    또한, 별칭 이름을 감싸는 『""』도 생략이 가능하지만,
--    『""』를 생략할 경우 별칭 내에서 공백 사용이 불가하다.
