...
-- ▼ TRIGGER(트리거 생성)
--    오전 8시 이전이거나 오후 6시 이후에는 작업을 수행할 수 없도록 처리
--    → 사용자 정의 예외를 발생시키도록 처리
--    트리거 명 : TRG_TEST1_DML
...
-- UPDATE : DELETE 그리고 INSERT가 결합된 형태
...
BEGIN
    UPDATE TBL_상품
    SET 재고수량 = 재고수량 + :NEW.입고수량                                         -- 재고수량 = 기존 재고수량 + 새로 입고되는 상품의 입고수량
    WHERE 상품코드 = :NEW.상품코드;                                                 -- 상품코드 = 새로 입고되는 상품의 상품코드
END;
-- Trigger TRG_IBGO이(가) 컴파일되었습니다.
-- ▷ IF문 없이 처리해도 INSERT 하나만 있다면 처리 가능!
--    하지만 INSERT 외의 UPDATE 혹은 DELETE가 들어올 때는 IF문을 사용해서 작성
*/
